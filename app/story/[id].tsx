...
import SaveCharacterModal, {
  ExtractedCharacter,
} from "@/components/SaveCharacterModal";
...
export default function StoryScreen() {
  ...
  const { profile, addCharacters } = useStore();
  ...
  // Extract characters after story loads (premium only, not temp stories)
  useEffect(() => {
    if (!isPremium || id === "temp" || !content || !title) return;
    ...
    const extractCharacters = async () => {
      try {
        ...
        if (error || !data?.characters) return;
        setExtractedCharacters(data.characters as ExtractedCharacter[]);
      } catch {
        // Silently ignore extraction errors — non-critical feature
      }
    };

    extractCharacters();
  }, [id, isPremium, content, title]);
  ...
  const handleScroll = (e: any) => {
    ...
    if (isAtEnd && !showFooter) {
      ...
      // Show save character modal when reader reaches the end (if characters extracted)
      if (isPremium && id !== "temp" && extractedCharacters.length > 0) {
  ...
  const handleShare = async () => {
    ...
    const link = hasLink
      ? `${SUPABASE_URL}/functions/v1/story-view?token=${share_token}`
      : null;
    await Share.share({
      title: title ?? "",
      message: link
  ...
  const handleCharactersSaved = (saved: Character[]) => {
    addCharacters(saved);
  };
      ...
      <SafeAreaView style={s.flex}>
        ...
        >
          ...
          >
            ...
            <Text style={[s.storyTitle, isTablet && s.storyTitleTablet]}>
              {title}
            </Text>
            ...
            {/* Paragraphes */}
            {paragraphs.map((p, i) => (
              <Text key={i} style={[s.para, isTablet && s.paraTablet]}>
                {p.trim()}
              </Text>
            ))}
        ...
        {showFooter && (
          ...
          >
            ...
            {isPremium && id !== "temp" && extractedCharacters.length > 0 && (
              <TouchableOpacity
